...
local git      = require('plugins.config.heirline.components.git')(utils, conditions, colors)

local file      = require('plugins.config.heirline.components.file')
local file_info = file.info(utils, conditions)
local file_type = file.type(utils)
...
local lsp_name   = lsp.name(conditions)
local diagnostic = lsp.diagnostic(conditions, colors)
...
local active_left_segment = {
    hl = { fg = 'grey' },
    vim_mode,
    utils.make_flexible_component(2, fold_method, null),
